...
namespace WebAppOnBus
{
    ...
    public class _2DModelHandler : IHttpHandler
    {
        ...
        //8台相机计算数据缓存
        public static SockPackage[] m_SockPack = new SockPackage[9];
        public static SockPackage[] SockPack
        {
            get
            {
                return m_SockPack;
            ...
            set
            {
                SockPackage[] m_SockPack = (SockPackage[])value.Clone();
            }
        }
                ...
                for(int k = 1; k <= 8; k++)
                {
                    ...
                    for(int j = 0; j < DataRows; j++)
                    {
                        try
                        {
                            ...
                            int px = (System.Convert.ToInt16(SockPack[k].s[j] * Math.Cos(BasicDegree - SockPack[k].ay[j]))) / ScaleParam + picWidth / 2;
                            int py = (System.Convert.ToInt16(SockPack[k].s[j] * Math.Sin(BasicDegree - SockPack[k].ay[j]))) / ScaleParam + picHeight / 2;
